...
class s3bucketnameRequest(BaseModel):
	bucket_name: str
...
@router.post("/store-s3-bucketname")
def store_s3_bucketname(data: s3bucketnameRequest):
	log_api_request(logger, "POST", "/store-s3-bucketname", region=data.bucket_name)
	try:
		...
		log_step(logger, "Updating AWS bucket name in .env", bucket_name=data.bucket_name)
		...
		# Refresh process environment so other APIs can use latest env variables immediately
		for k, v in env_dict.items():
			# ensure values are strings
			os.environ[k] = str(v)
		log_step(logger, "Refreshed process environment variables from .env", updated_keys=list(env_dict.keys()))

		log_success(logger, "AWS bucket name stored in .env and environment refreshed", bucket_name=data.bucket_name)
		log_api_response(logger, "POST", "/store-s3-bucketname", status_code=200)
		return {"message": ".env file in NEO-DSPM-HUB updated with AWS bucket name successfully"}
...
def store_aws_creds(data: AwsCredsRequest):
	log_api_request(logger, "POST", "/store-aws-creds", region=data.region)
	...
	try:
		...
		log_success(logger, "AWS credentials stored in .env", region=data.region)
